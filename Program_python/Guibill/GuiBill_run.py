...
if mn_funitures_pathindex == []:
    mn_funitures_pathindex = [
        match.start() for match in re.finditer(pattern[1], file_path)
    ]
...
def CreateGuibill():
    ...
    if len(Data["Delivery"]) == 1:
        ...
        if (Data["Delivery"]) == "3":
            ...
            Data["Delivery2"] = (
                datetime.datetime.today() + datetime.timedelta(days=35)
            ).strftime("%d/%m/%Y")
    ...
    template = cv2.imread(mn_funitures_path + "Bill/Template/TemplateBill.jpg")
    template[
        y_offset : y_offset + img.shape[0], x_offset : x_offset + img.shape[1]
    ] = img

    cv2.imwrite(mn_funitures_path + "Bill/PretoPaid/Pretopaid.jpg", template)
    image = Image.open(mn_funitures_path + "Bill/PretoPaid/Pretopaid.jpg")
    ...
    Summaryvalue = (int((Data["Price"])) * int((Data["Value"]))) - int(
        (Data["Discount"])
    )
    RemainValue = Summaryvalue - int((Data["Deposit"]))

    draw.text(
        (xyaxis["x_name"], xyaxis["y_name"]),
        Data["Name"] + "  (" + Data["Phone"] + ")",
        font=font,
        fill=(0, 0, 0, 0),
    )
    draw.text(
        (xyaxis["x_address"], xyaxis["y_address"]),
        Data["Address"],
        font=font2,
        fill=(0, 0, 0, 0),
    )
    draw.text(
        (xyaxis["x_order"], xyaxis["y_order"]),
        Data["Order"],
        font=font,
        fill=(0, 0, 0, 0),
    )
    draw.text(
        (xyaxis["x_delivery"], xyaxis["y_delivery"]),
        Data["Delivery"] + " -",
        font=font2,
        fill=(0, 0, 0, 0),
    )
    draw.text(
        (xyaxis["x_delivery2"], xyaxis["y_delivery2"]),
        Data["Delivery2"],
        font=font2,
        fill=(0, 0, 0, 0),
    )
    ...
    draw.text(
        (xyaxis["x_value"], xyaxis["y_value"]),
        Data["Value"],
        font=font,
        fill=(0, 0, 0, 0),
    )
    draw.text(
        (xyaxis["x_discount"], xyaxis["y_discount"]),
        Data["Discount"],
        font=font,
        fill=(0, 0, 0, 0),
    )
    draw.text(
        (xyaxis["x_price"], xyaxis["y_price"]),
        Data["Price"],
        font=font,
        fill=(0, 0, 0, 0),
    )
    draw.text(
        (xyaxis["x_priceafterdiscount"], xyaxis["y_priceafterdiscount"]),
        str(Summaryvalue),
        font=font,
        fill=(0, 0, 0, 0),
    )
    draw.text(
        (xyaxis["x_summary"], xyaxis["y_summary"]),
        str(Summaryvalue),
        font=font,
        fill=(0, 0, 0, 0),
    )
    ...
    draw.text(
        (xyaxis["x_deposit"], xyaxis["y_deposit"]),
        Data["Deposit"],
        font=font,
        fill=(0, 0, 0, 0),
    )
    ...
    draw.text(
        (xyaxis["x_remain"], xyaxis["y_remain"]),
        str(RemainValue),
        font=font,
        fill=(0, 0, 0, 0),
    )
    draw.text((100, 590), Data["Detail"], font=font, fill=(0, 0, 0, 0))
    image2 = image.copy()
    img_w = image2.width
    img_h = image2.height
    ...
    image2.paste(
        watermark,
        (int((img_w - watermarkW) / 2), int((img_h - watermarkH) / 2)),
        watermark,
    )
    image.save(mn_funitures_path + "Bill/PretoPaid/Pretopaid2.jpg")
    image2.save(mn_funitures_path + "Bill/PretoPaid/Pretopaid2_paid.jpg")
